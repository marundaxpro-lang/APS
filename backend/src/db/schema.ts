...
export const progressPhotos = pgTable(
  'progress_photos',
  {
    ...
    photoDate: timestamp('photo_date', { withTimezone: true }).notNull(),
...
/**
 * Achievement: Track user's achievements and milestones
 */
...
/**
 * FriendConnection: Track user friendships
 */
export const friendConnections = pgTable(
  'friend_connections',
  {
    ...
    status: text('status', { enum: ['pending', 'accepted', 'blocked'] }).default('pending'),
    createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow(),
    updatedAt: timestamp('updated_at', { withTimezone: true }).notNull().defaultNow().$onUpdate(() => new Date()),
  },
  (table) => [
...
/**
 * SocialPost: User's fitness posts and updates
 */
export const socialPosts = pgTable(
  ...
  (table) => [index('idx_user_social_posts').on(table.userId)]
);

/**
 * SocialPostLike: Track likes on posts
 */
...
/**
 * WorkoutSession: Track completed workout sessions
 */
...
export const nutritionLogs = pgTable(
  'nutrition_logs',
  {
    ...
    foodItems: jsonb('food_items').$type<
      Array<{
        name: string;
        calories: number;
        protein?: number;
